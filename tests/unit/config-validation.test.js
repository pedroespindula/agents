...
const workflowsReg = path.resolve('.opencode/workflows.json');

for (const p of [agentsReg, skillsReg, workflowsReg]) {
  if (!fs.existsSync(p)) fail(`Missing registry: ${p}`);
}

const agents = readJSON(agentsReg);
const skills = readJSON(skillsReg);
const workflows = readJSON(workflowsReg);
...
if (!Array.isArray(workflows.workflows)) fail('workflows registry malformed');
...
for (const w of workflows.workflows) {
  ...
  if (!fs.existsSync(p)) fail(`Workflow file missing: ${p}`);
}
